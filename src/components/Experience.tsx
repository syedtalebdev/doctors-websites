...
export default function Experience() {
  return (
    <section id="experience" className="bg-slate-50 border-y border-slate-200">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 sm:py-20 lg:py-24">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-12">
          {STATS.map((s) => (
            ...
            >
              <div className="font-display text-2xl sm:text-3xl font-bold text-primary-800 mb-1">
                {s.value}
              </div>
        ...
        <div className="space-y-4 sm:space-y-5">
          {ROLES.map((r) => (
            <div
              key={r.role}
              className="bg-white border border-slate-200 rounded-xl p-5 sm:p-6 lg:p-7"
            >
              <div className="flex flex-col lg:flex-row lg:items-start lg:gap-8">
                <div className="lg:w-56 flex-shrink-0 mb-3 lg:mb-0">
                  <div className="inline-flex items-center gap-2 text-xs sm:text-sm font-semibold text-primary-700 bg-primary-50 px-2.5 py-1 rounded-md">
                    <Briefcase className="w-3.5 h-3.5" />
                    {r.period}
                  </div>
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-display text-lg sm:text-xl font-bold text-slate-900 mb-0.5">
                    {r.role}
                  </h3>
                  <p className="text-slate-600 mb-3">{r.org}</p>
